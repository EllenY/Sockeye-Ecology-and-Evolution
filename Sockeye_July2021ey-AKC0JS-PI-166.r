...
ggplot(temp, aes(x=Lon, y=Lat, color=CPU)) +
  ...
  g <- gmap +
    geom_point(data=temp, aes(Lon, Lat, color=(temp), group=NULL),
...
print(x)
write.csv(x,"Full_sockeye_Pink.csv", row.names = TRUE)
...
fit_C = fit_model( "settings"=settings,
 ...
 check_fit(parameter_estimates, check_gradients = FALSE, quiet = FALSE)
#Check extrapolation grid total area....
colSums(fit_Cov$extrapolation_list$a_el)

#SAVE DATA =================================================================================
x=summary(fit_Cov$parameter_estimates$SD, select='report')
print(x)
write.csv(x,"Cal_AS1.csv", row.names = TRUE)

# PLOT RESULTs =============================================================================
plot_results(fit_Cov, n_cells=2000  )
...
X1_formula = ~ bs( lnSST, degree=3, intercept=FALSE)
X2_formula = ~ bs( lnSST, degree=3, intercept=FALSE)
X1_formula=~lnSST
X2_formula=~lnSST
...
X1<-data.frame(Lat,Lon, Year, lnSST,Calanusnls,Pollocknls) 
X1_formula = ~ lnSST+Calanusnls+Pollocknls
X2_formula = ~ lnSST+Calanusnls+Pollocknls
X1<-data.frame(Lat,Lon, Year, Calanusnls,Pollocknls) 
X1_formula = ~ Calanusnls+Pollocknls
X2_formula = ~ Calanusnls+Pollocknls
...
x=summary(fit$parameter_estimates$SD, select='report')
print(x)
write.csv(x,"Sock_SST.csv", row.names = TRUE)

# PLOT RESULTs =============================================================================
plot_results( fit, settings=settings3, plot_set=c(3,11,12,14,15), n_cells=2000)
head(fit$Report)
?plot_results
plot_variable(
 fit$Report$eta1_gct,
...
1 - (fit_Pink1$Report$deviance/fit_IntB$Report$deviance)
...
summary(D7$D)
0.001*4.793
0.001*11.49648 # J Pink
D7$D[D7$D<=.004793]<-"NA" #SST
D7$D[D7$D<=-0.6026889]<-"NA" # J Pink
11.49648+.6148
0.001*12.11128
-.6148+0.012111128
...
require("plyr")
  ...
  library(ggtext)
  pdf(file="FIGURE7A.pdf", height=8.5, width=11) 
  title_exp <- expression(log(kgU22C5km^{-2}))
#USE
  
  g <- gmap + 
   theme_bw()+
    geom_polygon(data = df2.WGScoor, aes(x = Lon, y = Lat, group = NULL), fill=NA,color="lightgray" ,size = 0.005) +
  geom_point(data=D9 , aes(Lon, Lat, color=((as.numeric(D))), group=NULL),  #Use log(D) for GRP to covert back from exp(GRP)
             na.rm=TRUE, size=1.25, stroke=0, shape=15) +facet_wrap('Year')+ 
    labs(color = title_exp, shape = title_exp)+
     # labs(color=paste('log(kg\U00B7km\u00b2)'))+
...
g2 <- gmap + 
  ...
  geom_point(data=D9 , aes(Lon, Lat, color=(D), group=NULL),  #Use log(D) for GRP to covert back from exp(GRP)
             na.rm=TRUE, size=2, stroke=0, shape=16) +facet_wrap('Year')+ 
 # labs(color = title_exp, shape = title_exp)+
 # labs(color=paste('log((#+1)·km)'))+
 # labs(color=paste('log(kg\U00B7km\u00b2+1)'))+
 # labs(color=paste('log(#\U00B7m\u00b2+1)'))+
 ...
 # labs(color=paste('Celsius'))+
  #labs(color=paste('log(kg\U00B7km\u00b2+1)'))+
  labs(y= "Latitude", x = "Longitude", cex=3)+
  labs(title="Sea surface temperature (20 m)")+
...
# Aleksey
###########
#### 5.1.4.4 VAST models estimates MAPS----

g_avg <- ggplot(world)+geom_sf(data=D_avg, aes(color=as.vector(B_pink), group=NULL),
                               ## These settings are necessary to avoid
                               ## overlplotting which is a problem here. May need
                               ## to be tweaked further.
                               size=2.8, stroke=0,shape=16)+
  #scale_color_gradientn(colours = colorspace::sequential_hcl( n = 7,palette="Lajolla",rev=FALSE),oob=scales::squish,limits=c(1,1.31))+
  scale_color_gradientn(colours = colorspace::divergingx_hcl(palette="RdGy", n=7, rev=TRUE),oob=scales::squish,limits=c(0,4))+
  geom_sf(data=REGS, fill="lightgreen", alpha=0,colour="black")+            
  geom_sf(data=world)+
  coord_sf(xlim = c(-1200000, 1300000), ylim = c(230000, 1900000),
           crs = "+proj=aea +lat_0=50 +lon_0=180 +lat_1=55 +lat_2=65 +x_0=0 +y_0=0 +datum=NAD83 +units=m +no_defs")+
  theme_bw() +
  theme(axis.line = element_line(colour = "black"),
        ...
        legend.title = element_text(angle = 90), panel.spacing = unit(0, "lines"))+
  guides(color = guide_colourbar(title.position = "right",
                                 title = "Biomass, ln(kg/km2)"))
g_avg
ggsave(filename=paste0("Pink_biomass_avg.png"), path=paste0(wd,"\\FIGS\\"),
       plot=g_avg, dpi = 300,  device = NULL)
...
#= expression(paste("x axis ", ring(A)^2)), y = "y axis")
 ...
 pdf(file="FIGURE 8.pdf", height=8.5, width=11) 
...
#Question: Are we specifying nonlinear covariate effects correctly?
...
#X1_formula = ~ bSpline( SSTnls, degree=3, intercept=FALSE)
#X2_formula = ~ bSpline( SSTnls, degree=3, intercept=FALSE)
#X2_formula = ~ bs( CPE, degree=3, intercept=FALSE)
#X1config_cp = array(3,dim=c(1,1)) #? for Splines
#X2config_cp = array(3,dim=c(1,1))
...
#pdf(file="SST_gamma_nl_cov_effects.pdf", height=11, width=8.5) 
# Plot 1st linear predictor
pred1 = Effect.fit_model( fit,
                         focal.predictors = c("lnSST"),
                         which_formula = "X1", 
                         xlevels = 100,
                         category_number=1)
plot1<-plot(pred1, xlab="Sea surface temperature", ylab=NULL, main="1st linear predictor")
plot1
# Plot 2nd linear predictor
pred2 = Effect.fit_model( fit2,
                          focal.predictors = c("lnSST"),
                          which_formula = "X2", 
                          xlevels = 100,
                          category_number=1)
plot2<-plot(pred2, xlab="", ylab=NULL, main="Positive catch rate")
plot2
?Effect.fit_model

plot(pred2)
grid.arrange(plot1, plot2,bottom=textGrob(expression(paste("SST))"))),left=textGrob(expression(paste("Linear predictor")),rot=90),nrow = 2, ncol=2)
dev.off()

#SST2
covariate_data_full = fit2$effects$covariate_data_full
catchability_data_full = fit2$effects$catchability_data_full

pdf(file="FIGURE 9.pdf", height=8.5, width=11) 
pred3 = Effect.fit_model( fit_SST,
                          focal.predictors = c("lnSST"),
                          which_formula = "X1", 
                          xlevels = 100)
plot3<-plot(pred3, xlab="Sea surface temperature (Celsius)", ylab=NULL, main="")
pred4 = Effect.fit_model( fit_Pink,
                          focal.predictors = c("Pinknls"),
                          which_formula = "X1", 
                          xlevels = 100)
plot4<-plot(pred4, xlab="Juvneile pink salmon (ln(kg+1))", ylab=NULL, main="")
...
######################################
g <- gmap +
  geom_point(data=D7, aes(Lon, Lat, color=(as.numeric(D)), group=NULL),
...
# FIGURE 2 USE  MAP OF STUDY AREA ============================================================================

#5/23/2022
#Matt Callahan matt.callahan@noaa.gov
...
pdf(file="FIGURE 2a.pdf", height=11, width=8.5) #This generates the figure as a hi-res (pub quality) tiff file. You can change the dimensions of the figure by changing the height and width arguments. You can change the resolution with the 'res' argument.
p<-ggplot()+
  geom_sf(data=russia)+
  geom_sf(data=AK)+
  ...
  theme(panel.background=element_rect(fill='white'),axis.title = element_text(size = 24),axis.text = element_text(size = 16),
...
legend("bottomright", seg.len = 3, cex = 0.7,
  ...
  ?reprex
  theme_bw()
 # tiff(file="FIGURE timeseries.tiff", height=11, width=8.5, units='in', pointsize=16,compression="lzw", res=1600) 
 pdf(file="FIGURE 3.pdf", height=11, width=8.5, paper='special')   
 ...
 p4<-ggplot(temp2, aes(x=Year, y=Easting)) +
  ...
  pdf(file="FIGURE 4.pdf", height=11, width=8.5, paper='special')   
  ...
  # Correlation matrix
  temp1<-data.frame(J_Sockeye, Northing, Easting, EAO, SST, lnCalanus,	lnPollock,	lnPink)
...
pairs.panels(temp1, method="pearson", hist.col="#00AFBB", density=TRUE)
  ...
  temp2<-data.frame(Year, J_Sockeye, Northing, Easting, EAO, SST, lnCalanus,	lnPollock,	lnPink)
